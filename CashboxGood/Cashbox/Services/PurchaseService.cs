    ...
    internal class PurchaseService : IPurchaseService
    {
        ...
        public void Purchase(int accountId, IEnumerable<int> productIds, decimal total)
        {
            using (var uow = _unitOfWorkFactory.Create())
            {
                ...
                // Check that account can't buy product if it's amount is 0.
                foreach (var product in products.Where(product => product.Amount == 0))
                {
                    throw new PurchaseException(PurchaseError.NotEnoughAmount, string.Format("Product {0} (id {1}) amount is 0!", product.Title, product.Id));
                }
